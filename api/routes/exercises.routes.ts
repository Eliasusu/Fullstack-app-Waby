...
exercisesRouter.get('/', async (req, res) => {
    ...
    const exercises = await ExerciseModel.getAll({ nameMuscleGroup });
    res.json(exercises);
});
...
exercisesRouter.get('/:idExercise', async (req, res) => {
    ...
    const exercise = await ExerciseModel.getById(idExercise);
...
exercisesRouter.delete('/:idExercise', async (req, res) => {
    const { idExercise } = req.params;
    const exercise = await ExerciseModel.delete(idExercise);
