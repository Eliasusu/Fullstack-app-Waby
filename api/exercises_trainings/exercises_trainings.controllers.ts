...
async function getOne(req: Request, res: Response) { 
    try {
        ...
        const exerciseTraining = await em.findOne(ExerciseTraining, { idTrainingExercise, training: { user: { idUser: req.body.user.id } }}, { populate: ['exercise', 'training', 'training.user'] });
...
async function update(req: Request, res: Response) {
    try {
        ...
        const exerciseTraining = await em.findOne(ExerciseTraining, { idTrainingExercise, training: { user: { idUser: req.body.user.id } } });
        if (!exerciseTraining) return res.status(404).json({ message: 'Exercise training not found' });
